...
const handleManualNextDriver = (state: RolesState) => {
  const nextDriver = getNextExcept(state.drivers, state.driver, state.navigator);
...
const handleManualNextNavigator = (state: RolesState) => {
  const nextNavigator = getNextExcept(state.navigators, state.navigator, state.driver);
...
const handleManualSwitchDriverAndNavigator = (state: RolesState) => {
  const driverIndexInNavigators = state.navigators.findIndex((n) => n === state.driver);
  const navigatorIndexInDrivers = state.drivers.findIndex((d) => d === state.navigator);
...
const manualUpdateHelper = (list: string[], nextList: string[], current: string, counterpart: string) => {
  current = current || '';
  ...
  if (
    leastCommonLength === 0 ||
    current.substring(0, leastCommonLength) !== next.substring(0, leastCommonLength) ||
    next === counterpart
  ) {
    next =
...
const handleManualUpdateDrivers = (state: RolesState, action: ManualUpdateDriversAction) => {
  const { list: nextDrivers, newline } = textToList(action.payload);

  const nextDriver = manualUpdateHelper(state.drivers, nextDrivers, state.driver, state.navigator);
...
const handleManualUpdateNavigators = (state: RolesState, action: ManualUpdateNavigatorsAction) => {
  const { list: nextNavigators, newline } = textToList(action.payload);

  let nextNavigator = manualUpdateHelper(state.navigators, nextNavigators, state.navigator, state.driver);
...
const handleTimerEnded = (state: RolesState) => {
  const nextDriver = state.drivers.length === 1 ? state.driver : getNextExcept(state.drivers, state.driver);

  const nextNavigator = getNextExcept(state.navigators, state.navigator, nextDriver);
...
const validate = (state: RolesState) => {
  const isValid = state.drivers.length >= 1 && state.navigators.length >= 2 && !!state.driver && !!state.navigator;
